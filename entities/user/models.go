...
func InsertUser(w http.ResponseWriter, r *http.Request) (Register, error) {
	rg := Register{}
	...
	var username string
	row := config.DB.QueryRow(userSQLStatement, username)
...
func SignIn(w http.ResponseWriter, r *http.Request) ([]Login, error) {
	if r.Method == "POST" {
		...
		case false:
			http.Redirect(w, r, "/login/user", http.StatusMovedPermanently)
			log.Println("Password and PassHash is not equal")
			break
		}
